...
steps:
  - uses: actions/checkout@v7
    with:
      ref: v0.6.0
  - uses: rubykatzen/flightdeck/.github/actions/build-bundle@v0.6.0
    with:
      paths: apps
...
steps:
  - uses: actions/checkout@v7
    with:
      ref: v0.6.0
  - uses: rubykatzen/flightdeck/.github/actions/build-apps-bundle@v0.6.0
    with:
      release-tag: v0.6.0
      token: ${{ secrets.GITHUB_TOKEN }}
```
...
jobs:
  deploy:
    uses: rubykatzen/flightdeck/.github/workflows/deploy-shared.yml@v0.6.0
    with:
...
The `@v0.6.0` pin on the `uses:` line only controls which ref runs `deploy/deploy.py` itself. `app-refs` entries are separate and don't have to match the workflow pin. <!-- x-release-please-version -->
