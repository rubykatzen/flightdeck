...
```yaml
- uses: rubykatzen/flightdeck/.github/actions/load-matrix@main
  id: matrix
  with:
    mode: deploy        # encrypt or deploy
    name: all            # default: all
...
Every file is fully validated before it is added to the matrix. Files may use either the `.yml` or `.yaml` extension, and their basename becomes the matrix item's `name`.
